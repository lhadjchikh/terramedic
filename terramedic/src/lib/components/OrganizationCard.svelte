...
<Card img={imageUrl} class="h-full transition-all duration-300 hover:shadow-lg border border-gray-200">
  <h3 class="text-terra-dark-blue mb-2 text-xl font-bold tracking-tight">{name}</h3>

  {#if tags.length > 0}
    <div class="mb-3 flex flex-wrap gap-1.5">
      {#each tags as tag}
        <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {tagStyle.bg} {tagStyle.text} {tagStyle.hover} transition-colors duration-200">
          {tag}
        </span>
      {/each}
    </div>
  ...
  <div class="mt-auto">
    <Button 
      ...
      class="w-full shadow-sm hover:shadow transition-all duration-200"
    >
      {actionText}
    </Button>
  </div>
